...
namespace Rivet {
  ...
  public:
    ...
    void init() {
      // for fileoutput
      file.open("/data/horse/ws/maml087d-workspace/rivet_anas/data/data.json", std::ios::trunc);
      
      // check if file is open
      if (!(file.is_open())){
      	std::cout << "couldnt open data.json -> terminating" << std::endl;
	std::exit(EXIT_FAILURE);
      ...
      // getting angular coeffs from coef datei
      myfile.open("/data/horse/ws/maml087d-workspace/rivet_anas/data/coeffs.txt"); 
      if (myfile.is_open()) {
		for (int i = 0; i<9; i++){
			string line;
			std::getline(myfile, line);
			coeffs[i] = std::stod(line);
		}
      }
      else { 
      	std::cout << "couldnt open coeff.txt -> terminating" << std::endl;
	std::exit(EXIT_FAILURE);
      }
    ...
    void analyze(const Event& event) {
      ...
      if (pass_VBFZtopo) {
		...
		// for (i == 0; i < event)
		  ...
		  _hist1->fill(vars.cos_theta, weight);
		  _hist2->fill(vars.phi, weight);
		  _hist3->fill(vars.cos_thetacheck, weight);
		  _hist4->fill(0);
...
//	std::cout << "-----------------------------------------------" << std::endl << std::endl;
    ...
    void inittemps (string name, int n) {
	    _t[name].label = name;
    ...
    void filltemps(string name, double theta, double cos, double phi) {
	    double normweight;
	    array<double, 9> pols;
...
//	    std::cout << "weights" << weight << std::endl;
    ...
    void scaletemps (string name, const double xsec){
...
//    }
    ...
    static double dxsec(double coeffs[9], double theta, double cos_theta, double phi) {
    ...
    static array<double, 9> polis(double coeffs[9], double theta, double cos_theta, double phi) {
	    double P8 = 1 + std::pow(cos_theta,2);
	    double P0 = (coeffs[0] * (1 - 3 * std::pow(cos_theta, 2))) / 2;
	    double P1 = coeffs[1] * sin(2 * theta) * cos(phi);
	    double P2 = coeffs[2] * std::pow(sin(theta),2) * cos(2 * phi) / 2;
